...
const ERROR_TEXT: Record<string, (args: readonly unknown[]) => string> = {
  ...
  DecompositionMismatch: () => "The outputs must sum to exactly the input's backing.",
  ...
  RestoreBackingMismatch: () =>
    "A piece changed denomination since the split, so the original cannot be restored.",
};
