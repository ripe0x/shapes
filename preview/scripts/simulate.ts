 ...
 * both a fully and a partially composed 100 ETH apex, one-tier and mixed-multiset splits,
 * restores, transfers, and redemptions.
 *
...
async function send(actor: number, fn: string, args: unknown[], value?: bigint) {
  const hash = await actors[actor].writeContract({
    address: dep.shapes,
    abi: shapesAbi,
...
async function main() {
  ...
  await mint(5, 8, 1); // a lone direct 100, untouched apex
  ...
  console.log("4. mid-tier compositions");
  ...
  console.log("5. splits: one-tier, mixed multiset, and deep");
  ...
  console.log("6. restores: two round trips; the step-5 splits above stay open");
  ...
  console.log(`the browsing wallet ${PRESENTS_TO} holds both apexes, a built 10, and a split piece`);
}
