...
resolver. Shape #0 is the backed collectible title exposed through `owner()`, but its holder has no
...
| Contract title | Shape #0 is atomically backed and minted to the deployer. `owner()` follows `ownerOf(0)`, returning zero whenever #0 is burned. Transfer, redemption, composition, decomposition and split affect it exactly like any other Shape; no authorization check reads `owner()`. |
