...
- **`burnBacking`** sets `isBlack` without touching the stack; `decompose`'s `isBlack` guard rejects it.
...
- **Non-reversal ops forfeit:** `split`/`redeem`/`burnBacking` on a survivor abandon its stack (records become inert). Consistent with "you chose not to un-merge first."
