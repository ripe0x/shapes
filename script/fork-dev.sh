...
# --disable-block-gas-limit: the simulation builds a genuine apex Complete (10,000 x 0.01
...
if [ -n "$FORK_URL" ]; then
  ...
  anvil --fork-url "$FORK_URL" --chain-id "$CHAIN_ID" --port "$PORT" --disable-block-gas-limit >/tmp/shapes-anvil.log 2>&1 &
else
  say "Starting local Anvil on $RPC (chain $CHAIN_ID)"
  anvil --chain-id "$CHAIN_ID" --port "$PORT" --disable-block-gas-limit >/tmp/shapes-anvil.log 2>&1 &
fi
ANVIL_PID=$!
