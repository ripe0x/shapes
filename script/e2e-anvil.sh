...
# A fresh Anvil sits at block 0, and the mint path reads `blockhash(block.number - 1)`, which
# underflows there. `forge script` simulates the whole run at the current block, so the deploy
# panics before it broadcasts anything. Only block 0 is affected; any real chain is far past it.
if [ "$(cast block-number --rpc-url "$RPC")" = "0" ]; then
  say "Mining one block (a fresh chain is at block 0, where the mint path underflows)"
  cast rpc anvil_mine 1 --rpc-url "$RPC" >/dev/null
fi

say "Deploying"
# Captured rather than streamed so the addresses can be parsed out, and printed in full on
# failure: a bare `grep` miss would otherwise kill the script under `pipefail` with no output.
...
# The deploy mints token 0 to the deployer, so the ids this script mints start after it. Read the
# counter rather than assuming: ids issue from 0 and `totalMinted` is a count, not the highest id.
FIRST_ID=$(cast call "$SHAPES" "totalMinted()(uint256)" --rpc-url "$RPC" | awk '{print $1}')
echo "  token 0 minted at deploy; this run mints ids $FIRST_ID and up"
...
# The nine mints run in denomination order from FIRST_ID, so 1 ETH is the fifth of them.
ETH1_ID=$((FIRST_ID + 4))
...
# Every live token except the 1 ETH one, already redeemed above. Token 0 is included: the deploy
# minted it, and leaving it out strands its backing and fails the reserve check below for a reason
# unrelated to redemption.
REST=""
for t in $(seq 0 $((FIRST_ID + 8))); do [ "$t" = "$ETH1_ID" ] && continue; REST="$REST,$t"; done
REST="[${REST#,}]"
