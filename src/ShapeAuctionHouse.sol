...
///      same terms, which is what lets the house sell a collection it knows nothing about.
///      `settle` and `cancelAuction` record an outcome and move nothing; `claimLot` is the single
///      path by which the lot leaves. A lot whose transfer reverts therefore blocks its own
///      delivery and nothing else: the seller still claims the winning cards and every outbid
///      bidder still withdraws, because those paths move Shapes alone. The lot's collection is
///      called from exactly two functions, `createAuction` and `claimLot`, whose callers are the
