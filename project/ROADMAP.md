...
Deliverables:
- MERGED IN PR #46, PENDING DEPLOYMENT/AUDIT: D-35 gives token #0 the fixed title `Shapes Collection Owner` and exclusive trait `Collection Owner: true`, with matching site identity and no authority change. The current Sepolia deployment still exposes the earlier renderer.
- MERGED IN PR #46, PENDING DEPLOYMENT/AUDIT: D-36 replaces the immutable 1% rate with 0.001 ETH per mainnet Shape and 0.00001 ETH per testnet Shape. Auction ETH bids pay per card created. Dual-profile reserve/lifecycle tests and size gates pass. R25's cheap direct high-tier rerolls require explicit audit and product signoff; the current Sepolia deployment does not exercise this ABI.
...
- External security audit required after issue #7, D-34, D-35 and D-36, with findings closed or accepted with rationale (D-16, D-17 folded in). `AUDIT_PROMPT_v6.md` pins exact merged flat-fee commit `1054db2455f7d6d3542a422130262bc872c34464`; selecting the independent auditor and completing the review remain open.
...
- COMPLETE FOR CURRENT RELEASE: deployment/lifecycle checks pass against a live mainnet fork; an exact-commit Sepolia deployment at the isolated testnet ladder is live from block 11613113. All eleven deployed contracts/libraries are Etherscan-verified. The isolated-schema Fly indexer and production Netlify site are cut over and chain-validated.
