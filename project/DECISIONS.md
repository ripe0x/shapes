...
- D-25 Legacy `IShapes` ERC-165 discovery after restoring `owner()`. OPEN, user-owned compatibility decision. Adding `owner()` directly to `IShapes` changes its interface id from pre-PR2 `0xbdcee955` to `0x306b220e`; the restored candidate currently advertises only the new id. This is a real compatibility break for consumers that probe the old capability, although no mainnet deployment exists. It is independent of the rejected `titleHolder()` substitution and does not require restoring that selector. Director recommendation: advertise both the current `type(IShapes).interfaceId` and the legacy `0xbdcee955` id, with regression tests, because the contract still implements every function in the pre-PR2 interface. Alternative: intentionally accept the prelaunch break and advertise only `0x306b220e`. Do not merge the correction until the user selects one.
