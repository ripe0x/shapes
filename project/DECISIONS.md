...
- D-05 Mainnet key and fee plan. `feeBps`/`feeRecipient` are immutable; D-24 selects a separate transferable `admin()` while Shape #0 represents powerless collectible ownership. Matters: irreversible if wrong; the admin key affects presentation surfaces and lock timing, and Shape #0 custody is a separate launch decision. Missing: user's multisig vs EOA intent, fee recipient address, initial Shape #0 recipient, and whether/when to lock renderer or renounce admin. Resolution: user decision, recorded here before P2 gate.
