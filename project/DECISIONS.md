...
- D-23 Title auction product line. claude/contract-title holds a complete unmerged exploration: ShapeTitleAuction.sol + interface + two test files + market-research prompt ("sell title without tokenising it"), 13 commits. PR #2 now proposes a competing tokenized title architecture through Shape #0. Matters: separate product decision, not hygiene; adopting D-24 likely supersedes this branch rather than making it follow-on work. Missing: user intent. Resolution: after D-24, user decides pursue the old non-tokenized auction separately, park it, or kill it; branch kept until then.

- D-24 Shape #0 contract title and separate admin (PR #2). OPEN, user-owned charter decision. PR #2 atomically mints backed Shape #0 to the deployer, makes its holder the transferable contract title, moves value-inert configuration authority to `admin()`, removes the external collector binding, and starts public minting/launch auction at #1. Matters: this changes constructor value flow, token numbering, ABI/interface detection, deployment addresses, the meaning of ownership, and Charter principle 5. Director recommendation: adopt the Shape #0 + separate-admin architecture in principle, but expose the title as `titleHolder()` or another explicit selector, not `owner()`. ERC-173 and Ownable tooling widely interpret `owner()` as the administrative controller; selector-only integrations cannot learn that this holder is powerless. If literal `owner()` is a core product requirement, user must explicitly accept that off-chain compatibility/security risk. Merge gate: user resolves this decision; PR #2 rebases after PR #1; legacy `IShapes` ERC-165 support and `IAdminControl` discovery are fixed; the false-positive seed test and direct #0 lifecycle coverage are fixed; stale docs are removed; canonical docs and Charter principle 5 are amended; full CI and deployment simulation pass. Resolution choices: adopt with the Director's rename, adopt literal `owner()` with explicit risk acceptance, or reject/park PR #2.
...
- D-05 Mainnet key and fee plan. `feeBps`/`feeRecipient` are immutable. Current main uses a single transferable EOA-style Ownable controller; PR #2 would replace it with a separate transferable `admin()` while Shape #0 carries the title. Matters: irreversible if wrong; the effective controller key affects presentation surfaces and lock timing, while a literal ceremonial `owner()` may mislead tooling. Missing: D-24 outcome, user's multisig vs EOA intent, fee recipient address, and whether/when to lock renderer or renounce the controller. Resolution: user decision, recorded here before P2 gate.
...
- D-16 External audit. Scope follows the release architecture: current Shapes + 5 linked libraries + lens + auction house/escrow; if D-24 lands, include the payable genesis constructor, Shape #0 lifecycle, admin/title separation, changed ABI, and the remaining 4 linked libraries after ContractCollectorOps removal. Deploy scripts, immutables, and ladder guards stay in scope. Gate for P2->P3. In-repo x-ray and PoCs are inputs, not substitutes.
