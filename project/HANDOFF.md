...
- D-30 reverses the branch-first deployment sequence by explicit user direction: releases deploy only from `main`. The site now catches missing `artist()` / `artistReleaseHash()` selectors, preserves the rest of the old Sepolia state load, and renders attribution as unavailable on that deployment. PR #4 is therefore safe to merge before deployment; after merge, deploy the exact `main` commit and land the new addresses/fromBlock as a small follow-up cutover.
...
- Merge PR #4 once its updated compatibility test and CI are green. From the resulting exact `main` commit, on explicit deployment go-ahead broadcast the rehearsed D-28 suite with the user-supplied Etherscan key kept only in process environment, verify every contract and exact payout/admin/direct-attribution readback, then run the guarded artist attestation using the actual Shapes creation transaction hash. Replace every address/fromBlock in `web/public/deployment.json` through a small follow-up, then rerun live site/CI checks. W-1, D-12, and W-4 remain separate P1 packets; D-08 remains the first experiment.
