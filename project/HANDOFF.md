...
- On explicit deployment go-ahead, broadcast the rehearsed D-28 suite from current `main` with the user-supplied Etherscan key kept only in process environment, verify every contract and exact payout/admin/direct-attribution readback, then run the guarded artist attestation using the actual Shapes creation transaction hash. Replace every address/fromBlock in `web/public/deployment.json` through a small follow-up, then rerun live site/CI checks. W-1, D-12, and W-4 remain separate P1 packets; D-08 remains the first experiment.
