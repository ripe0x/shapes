...
- D-11: the project-owned public Reown id is configured for Netlify production and preview builds. A deployed PR preview exposes WalletConnect and opens its scannable relay QR. The live check caught `cuer` 0.0.3 resolving incompatible `qr` 0.6.0; the branch pins 0.5.5 and adds a regression. A real iPhone then exposed a second configuration defect: only generic Browser Wallet and WalletConnect were registered. The stable refreshed preview at `https://wallet-options--shapes-onchain.netlify.app` shows Rainbow, Base, MetaMask and WalletConnect at both desktop and 390 x 844 viewports, and the QR still opens. Old hash-addressed Netlify previews are immutable and continue to show their old wallet inventory. Actual phone pairing plus Sepolia mint is still required.
...
1. Open the refreshed wallet-inventory preview on a real phone, confirm the named wallet choices, pair the intended mobile wallet, and complete a Sepolia mint. The public Reown id, connector and QR path are configured and verified.
