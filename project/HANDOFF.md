...
- D-11: the project-owned public Reown id is configured for Netlify production and preview builds. Live testing found and fixed the QR dependency break and generic-only wallet inventory. The next real iOS test reached Rainbow but failed with `No accounts found in approved namespaces`. The branch now bootstraps WalletConnect with Ethereum mainnet plus the deployment chain, labels chain 11155111 as Sepolia, pins every read/balance to the deployment chain, and switches there before writes. The stable preview remains `https://wallet-options--shapes-onchain.netlify.app`; actual Rainbow reconnection plus Sepolia mint is required before this remediation is accepted.
...
1. Open the refreshed stable preview on the same phone, discard the failed Rainbow pairing if it is cached, reconnect Rainbow, approve any Sepolia switch, and complete a Sepolia mint. This is the acceptance test for the namespace remediation.
