...
- Local P2 candidate on `codex/manage-shape-workspace` implements D-35 token #0 identity and D-36's immutable flat fee: 0.001 ETH per mainnet Shape and 0.00001 ETH per testnet Shape, including one fee per auction-bid card created. It is not merged or deployed. Default, testnet and deeper CI Foundry profiles each have 462 passing tests plus 4 expected fork skips, and all 4 fork tests pass against live Ethereum; Medusa passes 10/10 properties across 44,411 calls; the full Anvil lifecycle passes; 127 preview tests and preview/web builds pass. Shapes is 24,362/24,341 bytes with 214/235 bytes of margin; ShapeLens is 9,885/9,867; ShapeRenderer is 23,331/23,330; ShapeAuctionHouse is 7,939/7,930. `IShapes` is `0x86cf5406`. R25 flags the intended but material cheap-high-tier-reroll economics. The current Sepolia release still uses 100 bps and cannot validate this ABI.
...
- `AUDIT_PROMPT_v5.md` is historical. `AUDIT_PROMPT_v6.md` is the authoritative flat-fee brief, pinned to exact tested code commit `217498564f46667bc8fabce32be0a19c22d7e431`. The independent audit itself remains open.
