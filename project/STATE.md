...
Current phase: P1 entry gate. The corrected PR #2 owner/admin ABI is merged; D-27 admin-directed future mint-fee payout and D-28 direct artist attribution are implemented in draft PR #4 and await an atomic fresh Sepolia deployment, attestation, and metadata cutover (see `project/ROADMAP.md`). D-29 resolves the future position architecture without adding position execution or token freezing to Shapes.
