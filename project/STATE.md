...
Current phase: P1 entry gate. The corrected PR #2 owner/admin ABI is merged; D-27 admin-directed future mint-fee payout and D-28 direct artist attribution are complete in PR #4. Per D-30, the site tolerates the old Sepolia contract's missing attribution selectors so the implementation can merge first and the exact `main` commit can then be deployed, attested, and cut over in deployment metadata. D-29 resolves the future position architecture without adding position execution or token freezing to Shapes.

P0 GATE PASSED 2026-08-25. PR #1 merged as `5eec83d`; PR #2 merged as `7fca2b2`; corrective PR #3 merged as `bf5ae6b`. The adopted architecture is backed Shape #0 as transferable collectible ownership, exposed through `owner()`, with a separate bounded `admin()`. Admin controls presentation/discovery and may redirect future mint fees, but cannot change the fee rate or touch backing, redemption, accrued funds or token ownership. During PR #2 review the Director replaced `owner()` with `titleHolder()` without explicit user approval; PR #3 fully reverted that substitution before deployment. P1 entry now requires a fresh Sepolia deployment/readback.

PR #2's unrelated fixes remain accepted: the false-positive seed test reads the minted ids; valid Shape #0 split plus safe-transfer/self-custody paths are covered; genesis-block constructor simulation and genesis-aware scripts are fixed; and stale collector references are removed. The owner/admin correction merged in PR #3 as `bf5ae6b`. D-27 adds admin redirection of only future mint fees, with immutable `feeBps`. D-28 supersedes the pre-launch D-26 child design: immutable deployer attribution and its one-time deployment-bound EIP-712 signature are stored directly in Shapes, with no authority, statement, or two-step admin transfer. A stateless linked library holds only digest and signature-checking code. Token and collection metadata share one editable description; the separate collection renderer remains to preserve its exact animation. `IAdminControl` is `0xe135adbe` and `IShapes` is `0x926c1806`. Shapes is 24,103 bytes in the default build (473-byte EIP-170 margin) and 24,082 bytes in the testnet build (494-byte margin). D-25/R18 remain invalidated because no deployment or legacy consumer exposes this pre-mainnet architecture.
...
- Tests: the D-27/D-28 branch has 441 passing contract tests and 4 fork-only skips without MAINNET_RPC_URL in both default and testnet profiles. Focused direct-attribution tests cover independent EIP-712 encoding, chain/hash/deployment replay rejection, canonical EOA signatures, EIP-7702 delegated EOAs, and ERC-1271 including an empty valid signature. Fee-routing tests cover authorization, event/readback, future-fee-only behavior, zero rejection, renunciation freeze, and recovery from a reverting recipient without reserve impact. Shapes has 473 bytes of default-profile EIP-170 runtime margin and 494 bytes under testnet. Preview has 40 passing tests, including the pre-attribution deployment fallback, and builds; web lint/build, shell syntax, docs checks and local Anvil end-to-end deployment/signing/mint/redeem/auction flows pass. Coverage remains ~96.75% line / 90.95% branch per the last x-ray run. No Echidna/Medusa/Halmos.
