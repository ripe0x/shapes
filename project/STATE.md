...
Current phase: P1 entry gate. The corrected PR #2 owner/admin ABI is merged; D-26 artist attribution and D-27 admin-directed future mint-fee payout are implemented in draft PR #4 and await an atomic fresh Sepolia deployment, attestation, and metadata cutover (see `project/ROADMAP.md`).
...
PR #2's unrelated fixes remain accepted: the false-positive seed test reads the minted ids; valid Shape #0 split plus safe-transfer/self-custody paths are covered; genesis-block constructor simulation and genesis-aware scripts are fixed; and stale collector references are removed. The owner/admin correction merged in PR #3 as `bf5ae6b`. D-26 draft PR #4 adds immutable deployer attribution and a one-time deployment-bound EIP-712 release signature in a child contract, with no authority, statement, or two-step admin transfer. D-27 adds admin redirection of only future mint fees, with immutable `feeBps`; `IAdminControl` is `0xe135adbe` and `IShapes` remains `0xca355cbe`. Shapes is 24,369 bytes in the default build (207-byte EIP-170 margin) and 24,348 bytes in the testnet build (228-byte margin). D-25/R18 remain invalidated because no deployment or legacy consumer exposes this pre-mainnet architecture.
...
- Tests: the D-26/D-27 branch has 441 passing contract tests and 4 fork-only skips without MAINNET_RPC_URL in both default and testnet profiles. Focused attribution tests cover independent EIP-712 encoding, chain/hash/deployment replay rejection, canonical EOA signatures, EIP-7702 delegated EOAs, and ERC-1271 including an empty valid signature. Fee-routing tests cover authorization, event/readback, future-fee-only behavior, zero rejection, renunciation freeze, and recovery from a reverting recipient without reserve impact. Shapes has 207 bytes of default-profile EIP-170 runtime margin and 228 bytes under testnet. Coverage remains ~96.75% line / 90.95% branch per the last x-ray run. No Echidna/Medusa/Halmos.
