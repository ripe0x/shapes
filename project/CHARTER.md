...
5. Owner powers are value-inert: renderer, collection metadata, copy text, position resolver, contract-collector binding. None can touch ETH, backing, redemption, or token ownership. Renderer, resolver, and collector binding are independently one-way lockable.
...
9. `Shapes.sol` stays under EIP-170. Any core addition requires a `forge build --sizes` check; margin was 145 bytes on the reconciled PR #1/current-main tree at the last measurement.
